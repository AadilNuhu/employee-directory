...
const EditEmployee = ({id, onClose}:employeeFormProps) => {
    ...
    const employee: employeeType = employees.find((employee) => employee.id === id)
  return (
            ...
            <form className="space-y-4" onSubmit={handleSubmit}>
              ...
              <div className="flex gap-4">
                <div className="w-full">
                  ...
                  <select className="w-full rounded-lg border border-gray-300 px-4 py-3 outline-none focus:border-green-500" name="roles" 
                  value={employee?.role} 
                //   onChange={handleChange}
                   id="roles">
                      <option key={employee?.id} value={employee?.role} className="px-4 py-3">{employee?.role}</option>
                  </select>
                </div>
                ...
                value={employee?.department} 
                   id="department">
                    {/* {Departments.map((Dept) => ( */}
                      <option key={employee?.department} value={employee?.department} className="px-4 py-3">{employee?.department}</option>
                    {/* ))} */}
                  </select>
                </div>
